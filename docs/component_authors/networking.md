...
CF for Kubernetes has a goal for all network communication by System Components to be defined by Network Policies. The majority of these policies can be found in [config/networking/network-policy.yaml](../../config/networking/network-policy.yaml).
...
Let's say you are introducing a new component that needs to talk to UAA internally. You'll need to have a `NetworkPolicy` selecting UAA pods with `ingress` rules to allow traffic from your component by either [creating]("#creating-a-new-networkpolicy") a new ingress policy or [updating]("#updating-an-existing-networkpolicy") an existing ingress policy. Then, [you need to create an egress `NetworkPolicy`](#creating-an-egress-networkpolicy-for-your-component) selecting your component pods with `egress` rules to allow traffic to UAA.
...
Once you've tested it and everything works, make a PR to update the `NetworkPolicy` in [config/networking/network-policy.yaml](../../config/networking/network-policy.yaml)!
...
When you create a new namespace, in order to take advantage of all of Istio's features and communicate with other `Pods` that have Istio sidecars, your namespace must have an `istio-injection=enabled` label to turn on automatic [Sidecar Injection](https://istio.io/latest/docs/setup/additional-setup/sidecar-injection/). This makes it so the pods in your namespace will contain an [Istio sidecar proxy](https://istio.io/latest/docs/reference/config/networking/sidecar/) running alongside them. In CF for Kubernetes, Istio sidecar injection is added automatically by a `ytt` overlay [config/istio/add-istio-injection.yml](../../config/istio/add-istio-injection.yml).
